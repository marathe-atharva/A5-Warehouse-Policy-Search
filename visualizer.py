...
class GUI:
    ...
    MAX_WINDOW_SIZE = 0.80  # percent of full screen as decimal
    pygame.init()
    ...
    def __init__(
        self, part, test_case, state, total_num_actions, TEST_MODE=False, viewed=None
    ):
        pygame.display.init()
    ...
    def update(self, state, action, prev_loc=None, prev_box_locs=None, viewed=None):
        ...
        if "lift" in action:
            if not self.prev_illegal_move:
                ...
                if self.direction is None:
                    print(
                        f"Box {box} NOT adjacent to robot's location at {self.robot_position}!"
                    )
        ...
        for r in range(self.grid_num_rows):
            for c in range(self.grid_num_cols):
                ...
                if (
                    "*" in grid_symbol
                    and self.cost_so_far + self.ILLEGAL_MOVE_PENALTY <= total_cost
                    and action != "START"
                ):
                    self.draw_sprite("!", x, y)
                    failed_action = True
                ...
                if (r, c) == prev_loc:
                    if action in ("lift", "down"):
                        ...
                        if self.direction is not None:
                            self.draw_sprite(
                                WAREHOUSE_LEGEND[f"move {self.direction}"],
                                x,
                                y,
                                meta=None,
                            )
        ...
        action_msg = (
            f"Move Count: {self.total_actions_left * (1 if not self.TEST_MODE else -1)}"
        )
        ...
        pause = (
            True
            if (
                FRAME_RATE_PER_SECOND == 0
                or self.TEST_MODE
                or self.selected_action == "pause"
            )
            else False
        )
        quit_signal = False
                ...
                if event.type == KEYDOWN:
                    ...
                    if event.key == K_SPACE:
                        ...
                        self.selected_action = (
                            "pause" if self.selected_action != "pause" else None
                        )
                        break
                    ...
                    if self.TEST_MODE:
                        ...
                        if action_type in ("lift", "down"):
                            is_legal_move, destination_status = self.check_user_input(
    ...
    def check_user_input(self, action_type, direction):
        if action_type == "lift":
            dest_x, dest_y = destination = (
                DELTA_DIRECTIONS[DIRECTION_INDICES[direction]][0]
                + self.robot_position[0],
                DELTA_DIRECTIONS[DIRECTION_INDICES[direction]][1]
                + self.robot_position[1],
            )

            destination_is_adjacent = self.state._are_adjacent(
                self.robot_position, destination
            )
            destination_is_within_warehouse = self.state._is_within_warehouse(
                destination
            )
            destination_status = (
                self.grid[dest_x][dest_y]
                if destination_is_within_warehouse
                else "None!"
            )
            box_exists = destination_status.isalnum()

            is_legal_move = (
                destination_is_adjacent
                and box_exists
    ...
    def draw_sprite(self, key, x, y, meta=None):
        ...
        if key == "remove_box":
            img = pygame.transform.scale(
                img, [i // 2 for i in self.CELL_SIZE]
            ).convert_alpha()
            img.set_alpha(200)
            x += self.CELL_WIDTH // 4
            ...
            if meta in ("lift", "down"):
                img = pygame.transform.scale(
                    img, [i // 3 for i in self.CELL_SIZE]
                ).convert_alpha()
                x += self.CELL_WIDTH // 3
                y += self.CELL_WIDTH // 2
            else:
        ...
        if "*" in key and MASK_FLAG and not self.state.box_held:
            ...
            img = pygame.transform.scale(
                img, (self.CELL_WIDTH // 2, self.CELL_HEIGHT // 5)
            ).convert_alpha()
            x += self.CELL_WIDTH // 4
            y += int(self.CELL_HEIGHT * 0.35)
            self.screen.blit(img, (x, y))
        ...
        if key_val == "box" and key != "remove_box":
            ...
            self.screen.blit(
                text, (x + self.CELL_WIDTH // 5, y + self.CELL_HEIGHT // 6)
            )

        if key_val == "traversable" and self.part != "A":
            text = self.FONT.render(str(meta), True, BLACK)
            self.screen.blit(
                text, (x + self.CELL_WIDTH // 2.5, y + self.CELL_HEIGHT // 2.5)
            )
