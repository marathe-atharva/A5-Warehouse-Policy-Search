...
class State:
    ...
    def __init__(self, warehouse, warehouse_cost=None, robot_init=None):
        ...
        self.warehouse_cost = (
            warehouse_cost
            if warehouse_cost
            else [[0 for _ in row] for row in warehouse]
        )
    ...
    def update_according_to(self, action):
        ...
        if action_type == "move":
            ...
            raise Exception(
                f'RecklessRobotError: The boxes are fragile and should be put <down> with ease rather than being <drop>ped!: "{" ".join(action)}"'
            )
    ...
    def _attempt_move(self, direction):
        ...
        try:
            ...
            destination_is_adjacent = self._are_adjacent(
            ...
            is_legal_move = (
                ...
                and destination_is_traversable
            ...
            if is_legal_move:
                ...
                action_cost = (
                    ...
                    if len(direction) == 2
                    else self.ORTHOGONAL_MOVE_COST
                )
                self._increase_total_cost_by(self.ILLEGAL_MOVE_PENALTY + action_cost)

        except ValueError:
            raise Exception(
                "move direction must be 'n','ne','e','se','s','sw','w','nw' your move is: {}".format(
                    direction
                )
            )
        ...
        try:
            ...
            if is_legal_lift:
                ...
                self._increase_total_cost_by(
                    self.ILLEGAL_MOVE_PENALTY + self.BOX_LIFT_COST
                )
    ...
    def _attempt_down(self, direction):
        ...
        try:
            ...
            if is_legal_down:
                ...
                self._increase_total_cost_by(
                    self.ILLEGAL_MOVE_PENALTY + self.BOX_DOWN_COST
                )

        except ValueError:
            raise Exception(
                "improperly formatted down destination: {}".format(direction)
            )
        except IndexError:  # (row, col) not in warehouse
    ...
    def _are_adjacent(self, coordinates1, coordinates2):
        ...
        """
        return (
            self._are_horizontally_adjacent(coordinates1, coordinates2)
            or self._are_vertically_adjacent(coordinates1, coordinates2)
            or self._are_diagonally_adjacent(coordinates1, coordinates2)
        )
