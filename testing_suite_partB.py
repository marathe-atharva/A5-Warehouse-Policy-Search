...
def get_stochastic_probabilities(prob_as_intended):
    ...
    assert math.isclose(prob_as_intended + 2 * prob_slanted + 2 * prob_sideways, 1), (
        "Warning: probabilities don't sum to 1"
    )
    return outcomes
...
def display_policy(policy, values=None, description="Policy"):
    ...
    if values is not None:
        col_width += (
            len(
                str(
                    max(
                        (int(c) if c != float("inf") else 999)
                        for ir, r in enumerate(values)
                        for ic, c in enumerate(r)
                        if symbol_policy[ir][ic] != wall
                    )
                )
            )
            + 1
        ...
        formatted_policy = []
        int_values = [
            [(int(v) if v != float("inf") else "inf") for v in row] for row in values
        ]
        for v_row, s_row in zip(int_values, symbol_policy):
            values_and_symbols_row = (
                "".join(
                    [
    ...
    for i, r in enumerate(formatted_policy):
        ...
        ones = str(i)[-1]
        formatted_policy_with_row_index_labels.append(
            f"{tens.rjust(indent_spaces - 3)}{ones} |{r}| {tens}{ones}"
        )
...
class Submission:
    ...
    def _get_actions_using_policy(self, state, policy, stochastic_probabilities, seed):
        ...
        if policy and len(policy[0]) > 0:
            ...
            while len(actions_performed) < MAX_ACTIONS:
                ...
                if not isinstance(policy[i][j], str):
                    raise Exception(
                        f"Policy action must be a string.  Received: {policy[i][j]} at location ({i},{j})"
                    )
                elif len(policy[i][j]) == 0:
                    raise Exception(
                        f"No action provided in policy at location ({i},{j})"
                    )
                elif "-1" in policy[i][j]:
                    # Note to self, the initial robot location will never be on an actual wall
                    raise Exception(
                        f"Invalid policy action ('-1') at location ({i},{j}),  See PDF for valid actions (ie. move, lift, or down)"
                    )
                ...
                stochasticity_index_delta = random.choices(
                    population=[-2, -1, 0, 1, 2],
                    weights=stochastic_probabilities_in_index_order,
                )[0]
                stochastic_action = DIRECTIONS[
                    (intended_action_index + stochasticity_index_delta)
                    % len(DIRECTIONS)
                ]
            ...
            if not policy:
                raise Exception(
                    "No policy provided! The robot needs a policy in order to take actions."
                )
            else:
                raise Exception(
                    "The provided policy is empty, the robot was hoping to receive some guidance on the actions it should take!"
                )

        return actions_performed

    def compare_student_policy(
        self,
        test_case,
        warehouse,
        warehouse_cost,
        ...
        try:
            student_planner = DeliveryPlanner_PartB(
            ...
            to_box_policy, to_zone_policy, to_box_values, to_zone_values = (
                student_planner.generate_policies(debug=VERBOSE_FLAG)
            )
            ...
            if VERBOSE_FLAG:
                ...
                print(border)
                display_policy(
                    to_box_policy, to_box_values, description="To Box Policy"
                )
                display_policy(
                    to_zone_policy, to_zone_values, description="To Zone Policy"
                )

            score = []
            quit_signal = None
            for desc, policy in zip(
                ["to_box", "to_zone"], [to_box_policy, to_zone_policy]
            ):
                if desc == "to_zone":
                ...
                student_performed_actions = self._get_actions_using_policy(
                    copy.deepcopy(state),
                    policy,
                ...
                if VISUALIZE_FLAG and not quit_signal:
                    gui = GUI("B", test_case, state, len(student_performed_actions))
                    quit_signal = (
                        quit_signal if quit_signal is not None else gui.quit_signal
                    )
                    if quit_signal:
                        self.log(
                            "GUI received quit signal before executing any actions."
                        )
                        break
                ...
                student_performed_actions = "".join(
                    [symbol_lookup(action) for action in student_performed_actions]
                )

                max_num_actions = max(
                    len(correct_performed_actions[desc]), len(student_performed_actions)
                )
                student_performed_actions = student_performed_actions.ljust(
                    max_num_actions
                )
                correct_performed_actions[desc] = correct_performed_actions[desc].ljust(
                    max_num_actions
                )
                ...
                actions_correct = [
                    s == e
                    for s, e in zip(
                        student_performed_actions, correct_performed_actions[desc]
                    )
                ]
                score.append(0.5 if all(actions_correct) else 0)

                if VERBOSE_FLAG:
                    divider = ""  # can switch this to '|' for (possibly) easier readout alignment
                    correct_performed_actions_count = str(
                        len(correct_performed_actions[desc].strip())
                    ).rjust(2)
                    student_performed_actions_count = str(
                        len(student_performed_actions.strip())
                    ).rjust(2)

                    print(f"| {desc.replace('_', ' ').capitalize()}:")
                    print(
                        f"|  Correct actions performed [{correct_performed_actions_count}]: {divider.join(list(correct_performed_actions[desc]))}"
                    )
                    print(
                        f"|  Student actions performed [{student_performed_actions_count}]: {divider.join(list(student_performed_actions))}"
                    )
                    print(
                        f"|                     Differences: {divider.join([(' ' if ac else '^') for ac in actions_correct])}\n"
                    )
...
class PartBTestCase(unittest.TestCase):
    ...
    results = ["", "PART B TEST CASE RESULTS", "credit: [to box] + [to zone] = [total]"]
    SCORE_TEMPLATE = "\n".join(
        (
            "-----------",
            "Test Case {test_case}",
    ...
    def check_results(self, params):
        ...
        if not self.student_submission.submission_error.empty():
            ...
            self.results.append(
                self.FAIL_TEMPLATE.format(
                    message=error_message, output=logmsg, **params
                )
            )
        else:
            self.results.append(
                self.SCORE_TEMPLATE.format(
                    to_box_score=to_box_score,
        ...
        self._log(
            "test case {} credit: {}".format(
                params["test_case"], to_box_score + to_zone_score
            )
        )
        if error_message:
    ...
    def run_with_params(self, params):
        ...
        args = [
            ...
            params[
                "correct_performed_actions"
                if DISPLAY_UNICODE_ARROWS
                else "correct_performed_actions_alt_chars"
            ],
            params["seed"],
        ]
        if DEBUGGING_SINGLE_PROCESS:
            ...
            test_process = mproc.Process(
                target=self.student_submission.compare_student_policy, args=args
            )
    ...
    def test_case_01(self):
        ...
        params = {
            "test_case": 1,
            "warehouse": ["1..", ".#.", "..@"],
            "warehouse_cost": [[13, 5, 6], [10, w, 2], [2, 11, 2]],
            "todo": ["1"],
            "robot_init": (2, 2),
            "robot_init2": (0, 1),
            "seed": 7638,
