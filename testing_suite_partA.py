...
class Submission:
        ...
        if policy and len(policy[0]) > 0:
            ...
            while len(moves) < 1000:
                ...
                if not isinstance(policy[i][j], str):
                    raise Exception(
                        f"Policy action must be a string.  Received: {policy[i][j]} at location ({li},{lj})"
                    )
                elif len(policy[i][j]) == 0:
                    raise Exception(
                        f"No action provided in policy at location ({li},{lj})"
                    )
                elif "-1" in policy[i][j]:
                    # Note to self, the initial robot location will never be on an actual wall
                    raise Exception(
                        f"Invalid policy action ('-1') at location ({li},{lj}),  See PDF for valid actions (ie. move, lift, or down)"
                    )
                ...
                if not (0 <= i2 < len(policy)) or not (j2 < len(policy[0])):
                    raise Exception(
                        "Error in _get_actions_from_policy(): trying to move outside of warehouse at (i,j)=({},{}), action = {})".format(
                            i, j, policy[i][j]
                        )
                    )
    ...
    def execute_student_plan(
        self, test_case, warehouse, warehouse_cost, robot_initial_position, boxes_todo
    ):
        ...
        quit_signal = None
        try:
            student_planner = DeliveryPlanner_PartA(
                copy.deepcopy(warehouse),
                copy.deepcopy(warehouse_cost),
                copy.deepcopy(boxes_todo),
            )
            search_policy, deliver_policy = student_planner.generate_policies(
                debug=VERBOSE_FLAG
            )

            # First follow the policy from wherever the robot has been dropped to find the box
            search_actions = self._get_actions_from_policy(
                search_policy, robot_initial_position
            )
            ...
            if VISUALIZE_FLAG:
                gui = GUI("A", test_case, state, len(search_actions))
            ...
            if not quit_signal:
                ...
                deliver_actions = self._get_actions_from_policy(
                    deliver_policy, robot_initial_position
                )
                ...
                for action in deliver_actions:
                    ...
                    if len(delivered) > num_delivered:
                        ...
                        if last_box_delivered == next_box_to_deliver:
                            ...
                            raise Exception(
                                "box delivered out of order: box {} delivered instead of box {}".format(
...
class PartATestCase(unittest.TestCase):
        ...
        if not self.student_submission.submission_error.empty():
            ...
            self.results.append(
                self.SCORE_TEMPLATE.format(
                    cost=cost, score=score, output=logmsg, **params
                )
            )
    ...
    def run_with_params(self, params):
        ...
        if DEBUGGING_SINGLE_PROCESS:
            self.student_submission.execute_student_plan(
                params["test_case"],
                params["warehouse"],
            ...
            test_process = mproc.Process(
                target=self.student_submission.execute_student_plan,
                args=(
                    params["test_case"],
                    params["warehouse"],
                    params["warehouse_cost"],
    ...
    def test_case_01(self):
        ...
        params = {
            "test_case": 1,
            "warehouse": ["1..", ".#.", "..@"],
            "warehouse_cost": [[3, 5, 2], [10, w, 2], [2, 10, 2]],
            "todo": ["1"],
            "robot_init": (0, 2),
            "benchmark_cost": 23,
        }
    ...
    def test_case_02(self):
        ...
        params = {
            "test_case": 2,
            "warehouse": ["1..", ".#.", "..@"],
            "warehouse_cost": [[3, 25, 2], [10, w, 2], [2, 10, 2]],
            "todo": ["1"],
            "robot_init": (2, 1),
            "benchmark_cost": 37,
        }
