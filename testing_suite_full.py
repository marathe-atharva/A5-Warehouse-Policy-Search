...
def run_all(fout):
    partA_weight = 70  # 70% of total
    partB_weight = 30  # 30% of total
    totalScore = 0.0

    scores = []
    weights = [partA_weight, partB_weight]
    parts = ["A", "B"]

    for part_name, test_cases, weight in zip(
        parts, [PartATestCase, PartBTestCase], weights
    ):
        fout.write(f"{'-_' * 20}\n    part {part_name} starting    \n{'-_' * 20}\n")

        part_fout = io.StringIO()
        test_cases.fout = part_fout

        suite = unittest.TestSuite(
            unittest.TestLoader().loadTestsFromTestCase(test_cases)
        )
        ...
        fout.write(output)
        fout.write(
            f"part {part_name} result: {score:.02f}% [weight = {int(weight)}%]\n"
        )
    ...
    intTotalScore = int((totalScore) + 0.5)  # round score up to nearest integer
